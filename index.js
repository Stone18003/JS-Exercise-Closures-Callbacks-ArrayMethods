...
function lowerCaseStrings(strings) {
    return strings.map(name => name.toLowerCase());
}
...
function isItAnApple(strings) {
    return strings.map(name => name === 'apple');
}
...
function removeApple(strings) {
    return strings.filter(name => name !== 'apple');
}
...
function tallyUpDonations(runners) {
    return runners.reduce(function(prev, cur) {
 ...
 * 
 * counter2 is global scope.
 * 
 * 2. Which of the two uses a closure? How can you tell?
 * 
 * counter1 because it contains a function within a function. bwah!
 * 
 * 3. In what scenario would the counter1 code be preferable? In what scenario would counter2 be better? 
 *
 * counter1 in almost all scenarios because it doesn't rely on a global variable. counter2 might be useful for a small program where we need to keep track of a global count.
 * 
 */
